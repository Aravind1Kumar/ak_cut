  ...
  return (
    <header className="h-12 md:h-14 bg-dark-800 border-b border-dark-700 px-2 sm:px-4 flex items-center justify-between z-30 select-none shrink-0 overflow-x-auto scrollbar-none">
      ...
      <div className="flex items-center space-x-2 shrink-0">
        ...
        {/* Save Status Badge */}
        <div className="hidden lg:flex items-center space-x-1 px-2 py-1 rounded bg-dark-900/60 border border-dark-700 text-[10px] font-mono text-gray-400">
          {saveStatus === 'Saved' ? (
            <>
            ...
            <>
              ...
              <span className="text-amber-400">{saveStatus}</span>
            </>
          )}
        </div>
      ...
      <div className="flex items-center space-x-1 sm:space-x-2 shrink-0">
        ...
        <div className="h-4 w-[1px] bg-dark-700 mx-1 hidden sm:block" />

        {/* Responsive Layout Mode Switcher */}
        <div className="flex items-center bg-dark-900/80 p-0.5 rounded-lg border border-dark-700 text-[10px] md:text-xs">
          <button
            onClick={() => setLayoutMode('auto')}
            className={`px-1.5 md:px-2.5 py-1 rounded-md transition font-medium flex items-center space-x-1 ${
              layoutMode === 'auto'
        ...
        <div className="flex items-center bg-dark-900/80 p-0.5 rounded-lg border border-dark-700 text-[10px] md:text-xs">
          {aspectRatios.map((item) => (
            <button
              ...
              className={`px-1.5 md:px-2.5 py-1 rounded-md transition flex items-center space-x-1 font-medium ${
      ...
      <div className="flex items-center space-x-2 shrink-0">
        <button
          ...
          className="flex items-center space-x-1.5 px-3 md:px-4 py-1.5 md:py-2 text-xs font-bold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 rounded-xl shadow-lg shadow-cyan-500/20 transition transform active:scale-95"
        >
